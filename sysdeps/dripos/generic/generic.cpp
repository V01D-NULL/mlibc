...
void sys_exit(int status) {
  asm volatile("syscall" : : "a"(12), "D"(status) : "rcx", "r11", "rdx");
}
#endif
...
int sys_vm_map(void *hint, size_t size, int prot, int flags, int fd,
  ...
  // mlibc::infoLogger() << "calling sys_vm_map with size: " << size <<
  // frg::endlog;
...
int sys_futex_wake(int *pointer) {
  uint64_t err;
  asm volatile("syscall" : "=d"(err) : "a"(65), "D"(pointer) : "rcx", "r11");
...
int sys_fork(pid_t *child) {
  ...
  asm volatile("syscall" : "=a"(ret), "=d"(sys_errno) : "a"(57) : "rcx", "r11");
